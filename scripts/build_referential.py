...
def build_referential(raw_dir):
    ...
    original_rows = read_csv(raw_dir / "Original_Dataset.csv", "utf-8-sig")
    description_rows = read_csv(raw_dir / "Disease_Description.csv", "utf-8-sig")
    mapping_rows = read_csv(raw_dir / "Doctor_Versus_Disease.csv", "latin1")
...
def main():
    ...
    write_csv(rows, Path("data/processed/medical_referential.csv"))
    print(f"Wrote medical_referential.csv with {len(rows)} rows.")
